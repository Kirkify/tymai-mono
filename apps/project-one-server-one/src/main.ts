...
import { RootModule } from '@mn/project-one/server/server-one/modules/root';
import { HttpExceptionFilter } from '@mn/project-one/server/shared/exceptions';
import { appConfig } from '@mn/project-one/server/server-one/configs';
...
async function bootstrap() {
  ...
  const openApiConfig = new DocumentBuilder()
    ...
    .setDescription('Project One Server One API description')
    .setVersion('1.0')
    .build();
